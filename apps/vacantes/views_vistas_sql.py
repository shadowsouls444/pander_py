...
class VVacanteListView(APIView):
    """
    GET /api/v/companias/{compania}/vacantes/
        ?estado=1  ?ind_publicada=true  ?unidad=2
    """
    def get(self, request, compania):
        qs = VVacante.objects.filter(compania=compania)
...
class VVacanteDetailView(APIView):
    """GET /api/v/companias/{compania}/vacantes/{id}/"""
    def get(self, request, compania, id):
        obj = get_object_or_404(VVacante, id=id, compania=compania)
